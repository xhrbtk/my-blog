...
- 把js放到body里最后的位置，放置加载js阻塞页面
5,静态资源压缩
- 图片，css，js在发布前要压缩
6.静态资源使用多个域名
- 对于图片，css，js，可使用几个域名，可以并发加载
7. 静态资源使用cdn存储
- 用户与你网站服务器的的接近程度会影响响应时间的长短。可以吧静态资源放到内容分发网络
8.预加载
- 在某个功能还没展现时，在空闲时间预先加载相关图片或js代码
9dom操作优化
- 缓存已经访问过的有关元素
- 线下更新完节点之后再将它添加到文档树中
- 避免使用js来修改页面布局
...
<pre style="margin: 0px; padding: 0px; white-space: pre-wrap; word-wrap: break-word; font-family: &quot;Courier New&quot; !important; font-size: 12px !important;">HTTP/1.1 301 Moved Permanently
...
　　有一种常见的极其浪费资源的重定向，而且web开发人员一般都意识不到这一点，就是URL尾部缺少一个斜线的时候。例如，跳转到[http://astrology.yahoo.com/astrology](http://astrology.yahoo.com/astrology)会返回一个重定向到[http://astrology.yahoo.com/astrology/](http://astrology.yahoo.com/astrology/)的301响应（注意添在尾部的斜线）。在Apache中可以用`Alias`，`mod_rewrite`或者`DirectorySlash`指令来取消不必要的重定向。
...
[回到顶部](https://www.cnblogs.com/xianyulaodi/p/5755079.html#_labelTop)
...
<pre style="margin: 0px; padding: 0px; white-space: pre-wrap; word-wrap: break-word; font-family: &quot;Courier New&quot; !important; font-size: 12px !important;">GET /i/yahoo.gif HTTP/1.1
      Host: us.yimg.com
...
<pre style="margin: 0px; padding: 0px; white-space: pre-wrap; word-wrap: break-word; font-family: &quot;Courier New&quot; !important; font-size: 12px !important;">ExpiresDefault "access plus 10 years"</pre>
